-- IRONCLAD HARDENED BOOKING RPC (v3.3)
-- Adds Idempotency, Audit Logging, and Strict Atomic Locking.
...
BEGIN
  -- 1. IDEMPOTENCY CHECK
  -- If we've already successfully processed this request, return the previous result.
  ...
  -- 3. DEFENSIVE OWNERSHIP VERIFICATION
  IF NOT EXISTS (SELECT 1 FROM public.services WHERE id = p_service_id AND org_id = p_org_id) THEN
  ...
  -- We use a subquery with FOR UPDATE to prevent race conditions on staff slots.
  
  IF p_staff_id IS NOT NULL THEN
     -- Specific Staff Mode
     -- Lock the staff record and any existing appointments for this staff/date to ensure isolation.
  ...
  ELSE
     -- "Any Professional" Mode: Lock potential staff members
     -- This is complex for "Any", but we'll try to find one and lock it.
     ...
     ORDER BY RANDOM()
     LIMIT 1
     FOR UPDATE OF s; -- LOCK the selected staff row
  ...
  -- Get staff and service snapshots
  ...
  SELECT 
    ...
    p_date, p_time_slot, p_notes, p_duration_minutes, p_buffer_minutes, 'CONFIRMED',
    s.name, s.price, v_staff_name
  FROM public.services s
  WHERE s.id = p_service_id
  RETURNING id INTO v_appointment_id;

  -- 8. FINAL AUDIT LOG (CONFIRMED)
  PERFORM public.log_booking_event(
...
EXCEPTION WHEN OTHERS THEN
  -- Log failure unless it's a specific handled case if needed
...
GRANT EXECUTE ON FUNCTION public.create_appointment_v3 TO authenticated, service_role, anon;
