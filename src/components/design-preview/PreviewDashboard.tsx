                        ...
                        <div className="absolute inset-0 flex flex-col overflow-y-auto bg-gray-50 pb-24">
                            ...
                            <main className="p-6 space-y-6">
                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                                    {[
                                        { label: 'Revenue', val: '$4,250', change: '+12%', icon: DollarSign, color: 'text-green-600', bg: 'bg-green-100' },
                                        { label: 'Bookings', val: '48', change: '+5%', icon: CalendarDays, color: 'text-blue-600', bg: 'bg-blue-100' },
                                        { label: 'New Clients', val: '12', change: '+8%', icon: Users2, color: 'text-purple-600', bg: 'bg-purple-100' },
                                        { label: 'Avg Value', val: '$85', change: '-2%', icon: TrendingUp, color: 'text-orange-600', bg: 'bg-orange-100' },
                                    ].map((stat, i) => (
                                        <div key={i} className={`${styles.card} p-5`}>
                                            <div className="flex justify-between items-start mb-3">
                                                <div className={`p-2 rounded-xl ${stat.bg} ${stat.color}`}>
                                                    <stat.icon className="w-5 h-5" />
                                                </div>
                                                <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${stat.change.startsWith('+') ? 'bg-green-50 text-green-500' : 'bg-red-50 text-red-500'}`}>{stat.change}</span>
                                            </div>
                                            <h3 className="text-3xl font-black text-gray-900 tracking-tighter">{stat.val}</h3>
                                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">{stat.label}</p>
                                        </div>
                                    ))}
                                </div>
                                <div className={`${styles.card} p-6 h-[300px] flex flex-col`}>
                                    <h3 className="text-lg font-black text-gray-900 mb-4">Revenue</h3>
                                    <div className="flex-1 w-full bg-gray-50 rounded-2xl flex items-center justify-center text-gray-300 font-bold uppercase tracking-widest text-[10px]">
                                        Chart Visualization
                                    </div>
                                </div>
                            </main>
