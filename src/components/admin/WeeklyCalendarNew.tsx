...
const PulseStyle = () => (
    <style dangerouslySetInnerHTML={{
        ...
        @keyframes gravityPulse {
            0% { transform: scale(1); opacity: 1; box-shadow: 0 0 0 0 var(--primary-color); }
...
export default function PreviewCalendar({ hideNav = false, onTabChange }: { hideNav?: boolean, onTabChange?: (tab: 'calendar' | 'stats' | 'settings') => void }) {
    ...
    const staffMembers = [
        { id: 1, name: 'Sarah', color: 'bg-primary-50', border: 'border-primary-500', text: 'text-primary-700' },
    ...
    const renderNewBookingModal = () => (
        <div className="absolute inset-0 z-[100] flex flex-col bg-[#F2F2F7]">
            ...
            <div className="flex justify-between items-center px-6 pt-10 pb-4 bg-[#F2F2F7]/95 backdrop-blur-xl border-b border-gray-200 shrink-0 sticky top-0 z-20">
                <button onClick={() => setIsModalOpen(false)} className="text-primary-600 text-[17px] hover:opacity-70 transition-opacity">Cancel</button>
                <span className="font-bold text-[17px] text-black">New Event</span>
                <button onClick={handleSaveBooking} className="font-bold text-primary-600 text-[17px] hover:opacity-70 transition-opacity">Add</button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar pb-10">
                ...
                <div className="bg-white mt-4 mx-4 rounded-xl overflow-hidden shadow-sm border border-gray-100 p-4">
                    ...
                    <div className="mt-6">
                        ...
                        <div className="h-14 bg-gray-50 rounded-2xl border border-gray-100 relative flex items-center justify-center p-2">
                            <div className="flex-1 max-w-[140px] h-9 bg-primary-50 border-[1.5px] border-primary-400 rounded-xl flex items-center justify-center shadow-sm">
                                <span className="text-[11px] font-bold text-primary-600">New Event</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white mt-4 mx-4 rounded-xl overflow-hidden shadow-sm border border-gray-100">
                    {[
                        ...
                        { label: 'Staff Member', value: 'Sarah', color: 'bg-primary-500' },
                        { label: 'Calendar', value: 'Personal', color: 'bg-primary-600' },
    ...
    const renderYearBlock = (year: number) => (
        <div key={year} className="mb-12">
            <h2 className={`text-3xl font-bold px-4 mb-4 border-b border-gray-50/0 ${year === 2025 ? 'text-primary-600' : 'text-gray-900'}`}>{year}</h2>
            <div className="grid grid-cols-3 gap-x-2 gap-y-6 px-2">
                {months.map((m, i) => {
                    ...
                    return (
                        ...
                        >
                            <h3 className={`text-[13px] font-bold pl-0.5 ${i === 11 && year === 2025 ? 'text-primary-600' : 'text-gray-900'}`}>{m}</h3>
                            <div className="grid grid-cols-7 gap-y-[2px] gap-x-0 pointer-events-none">
                                ...
                                {Array.from({ length: days }).map((_, d) => {
                                    ...
                                    return (
                                        <div key={d} className={`w-full h-[10px] flex items-center justify-center text-[7px] font-medium leading-none ${isToday ? 'bg-primary-600 text-white rounded-full' : 'text-gray-800'}`}>
                                            {dayNum}
                                        </div>
                                    );
                                })}
            ...
            <div className="grid grid-cols-7 auto-rows-fr">
                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                    ...
                    return (
                        ...
                        >
                            <span className={`absolute top-1 left-1/2 -translate-x-1/2 text-sm font-medium ${monthName === 'December' && dayNum === 27 ? 'bg-primary-600 text-white w-7 h-7 rounded-full flex items-center justify-center shadow-sm -mt-0.5' : 'text-gray-900'}`}>
                                {dayNum}
                            </span>
                                ...
                                <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center justify-center">
                                    {eventCount === 1 ? (
                                        ...
                                        <div className="flex items-center gap-[1px] px-1 py-0.5 rounded-full bg-gray-100/80">
                                            ...
                                            <span className="text-[9px] font-black text-primary-600 leading-none mb-[0.5px] tracking-tight">{eventCount}</span>
                                        </div>
                                    )}
                                </div>
                            )}
                        ...
                        >
                            ...
                            {selectedDate === 27 && (
                                <div className="absolute w-full z-30 pointer-events-none" style={{ top: `847px` }}>
                                    <div className="w-full h-[1px] bg-primary-600 shadow-[0_0_8px_var(--primary-color)]"></div>
                            ...
                            {mockEvents.filter(e => (e.type === 'personal' || e.type === 'new')).map(event => (
                                <div
                                    ...
                                    className="absolute left-2 right-2 rounded-[4px] bg-primary-50 border-l-[3px] border-primary-500 p-2 text-primary-900 overflow-hidden cursor-pointer z-10 shadow-sm animate-in zoom-in-95 duration-200"
                                    style={{ top: `${event.start * 60}px`, height: `${event.duration * 60}px` }}
                                >
                                    <div className="text-sm font-bold leading-tight text-primary-700">{event.title}</div>
        ...
        <div className="w-full h-full bg-white text-gray-900 flex flex-col font-sans overflow-hidden select-none relative">
            ...
            <header className="pt-6 pb-2 px-5 bg-[#F2F2F7]/90 backdrop-blur-md sticky top-0 z-50 flex flex-col shrink-0 transition-all">
                ...
                <div className="h-6 flex items-start">
                    {calendarLevel === 'month' && (
                        <div className="flex items-center gap-1 text-primary-600 cursor-pointer active:opacity-50" onClick={() => { setDirection('backward'); setCalendarLevel('year'); }}>
                            <ChevronLeft className="w-5 h-5 -ml-1.5" strokeWidth={2.5} />
                            <span className="text-[17px] font-normal">2025</span>
                        </div>
                    )}
                    {calendarLevel === 'day' && (
                        <div className="flex items-center gap-1 text-primary-600 cursor-pointer active:opacity-50" onClick={() => { setDirection('backward'); setCalendarLevel('month'); }}>
                            <ChevronLeft className="w-5 h-5 -ml-1.5" strokeWidth={2.5} />
                ...
                <div className="flex items-end justify-between mt-1">
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            className="text-sm font-semibold text-primary-600 bg-primary-100/50 px-3 py-1 rounded-full hover:bg-primary-100 transition-colors mb-1.5"
                    ...
                    <div className="flex items-center gap-2.5 mb-2">
                        ...
                        <CalendarIcon className="w-6 h-6 text-primary-600" strokeWidth={2} />
                        <button
                            className="text-primary-600"
                        ...
                        {weekDates.map((date) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {isToday && !isSelected && <div className="w-1 h-1 rounded-full bg-primary-600 mt-1"></div>}
                                </div>
                            );
                        })}
                    </div>
            ...
            {/* MOCK TAB BAR */}
            {
                !hideNav && (
                    <div className="h-[83px] bg-white border-t border-gray-200 flex items-start justify-around pt-3 shrink-0 pb-8 sticky bottom-0 z-50">
                        <div className="flex flex-col items-center gap-1 text-primary-600 cursor-pointer" onClick={() => onTabChange?.('calendar')}>
